...
- **新增 Doc 目录** - 框架架构文档系统
    - Core 模块架构文档 (19 个子模块): async, attributes, audio, blackboard, broker, context, datatable, frameworksettings, fsm, graphs, logging, network, pooling, reactive, resources, scene, serialization, simulation, time, ui
    - Unity 模块架构文档：bootstrapper, modules
    - 文档遵循统一结构：设计理念 → 软件架构 → API 使用指南 → 正确示例 → 错误示例

#### 性能测试

- **新增 AsakiContext 性能测试** - 验证无锁读取和写时复制性能
    - 测试高并发场景下的服务解析性能
    - 验证 Copy-On-Write 机制的有效性
...
- **优化 AsakiUIManageService 关闭窗口逻辑** - 防止重复关闭和竞争条件
    - 将关键方法标记为 virtual 以支持测试继承
    - 改进日志信息和错误处理
    - 新增 Phase1 测试用例，覆盖替换窗口、返回值和重复关闭等场景

#### API 推荐用法更新

- **新增类型安全的 OpenAsync 泛型接口** - 推荐用于打开窗口
    - `OpenAsync<TWindow, TArg>(TArg arg)` - 类型安全的异步窗口打开方法
    - 替代旧的 `OpenAsync(Type windowType, object arg)` 非泛型版本
    - 提供编译时类型检查和更好的 IDE 智能提示

- **新增类型安全的 Back 泛型接口** - 推荐用于关闭窗口并返回值
    - `Back<TResult>(TResult result)` - 类型安全的返回值方法
    - 替代旧的 `Back(object result)` 非泛型版本
    - 避免装箱/拆箱操作，提升性能
...
- **非泛型 object 路径接口将逐步废弃** - 迁移指南
    ...
    - 迁移建议：
        - `OpenAsync(typeof(MyWindow), arg)` → `OpenAsync<MyWindow, MyArg>(arg)`
        - `Back(result)` → `Back<SpecificType>(result)`
...
- **配置优先级** - 支持多层级控制
    - 第一优先级：代码动态控制（`DiagnosticsEnabled` 属性）
    - 第二优先级：AsakiUIConfig 配置项（`EnableDiagnostics`）
    - 默认行为：条件编译（编辑器/开发版本启用，发布版本禁用）

### Fixed

#### UI 系统

- **修复关闭窗口时的竞争条件** - 确保同一窗口不会被重复关闭
- **修复 Back 返回值问题** - 确保返回值正确传递
- **修复 Replace 一致性问题** - 确保替换操作的一致性
