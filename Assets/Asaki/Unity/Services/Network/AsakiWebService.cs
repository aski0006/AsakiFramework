...
namespace Asaki.Unity.Services.Network
{
    ...
    public class AsakiWebService : IAsakiWebService
    {
        ...
        private readonly HashSet<IAsakiWebInterceptor> _interceptors =
            new HashSet<IAsakiWebInterceptor>();
        ...
        public void Setup(AsakiWebConfig config)
        {
            ...
            if (config.InitialInterceptors == null)
                return;
        ...
        /// </remarks>
        public async UniTask<TResponse> PostFormAsync<TResponse>(
            string apiPath,
            WWWForm form,
            CancellationToken token = default(CancellationToken)
        )
        ...
        {
            if (_isDisposed)
                throw new ObjectDisposedException(nameof(AsakiWebService));
        }
        ...
        {
            if (
                apiPath.StartsWith("http", StringComparison.OrdinalIgnoreCase)
        ...
        /// </remarks>
        private byte[] SerializeRequestToBytes<T>(T body)
            where T : IAsakiSavable
        {
        ...
        /// </remarks>
        private T ProcessResponse<T>(UnityWebRequest uwr)
