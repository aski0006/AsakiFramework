...
namespace Asaki.Unity.Services.DataTable
{
    ...
    /// <para>核心功能：</para>
    /// <list type="bullet">
    ///     <item>监控 <c>StreamingAssets/FrameworkSettings</c> 目录下的 <c>*.csv</c> 文件</item>
    ...
    ///     <item>仅监控 <c>StreamingAssets/FrameworkSettings</c> 目录，其他路径需修改代码</item>
    ...
    ///
    /// // 典型工作流程：
    /// // 1. 修改 StreamingAssets/FrameworkSettings/ItemConfig.csv
    /// // 2. 保存文件（Ctrl+S）
    /// // 3. 观察 Unity 控制台输出 "[AsakiFrameworkSetting] File Changed: ItemConfig, Reloading..."
    ...
    {
        /// <summary>
        /// 配置文件监控根目录，固定为 <c>Application.streamingAssetsPath/FrameworkSettings</c>。
        /// </summary>
        /// <remarks>
        /// <para>路径解析：</para>
        /// <list type="bullet">
        ///     <item>Windows: <c>%AppData%/../LocalLow/CompanyName/ProductName/StreamingAssets/FrameworkSettings</c></item>
        ///     <item>macOS: <c>~/Library/Application Support/CompanyName/ProductName/StreamingAssets/FrameworkSettings</c></item>
        ///     <item>Editor: <c>Assets/StreamingAssets/FrameworkSettings</c></item>
        /// </list>
        ...
        private void Start()
        {
            ...
            _watchPath = Path.Combine(Application.streamingAssetsPath, "FrameworkSettings");
            if (!Directory.Exists(_watchPath))
                return;

            InitWatcher();
            ALog.Info($"[AsakiFrameworkSetting] Hot Reload Watcher Started: {_watchPath}");
        }
        ...
        private void ReloadConfig(string filePath)
        {
            ...
            if (configType != null)
            {
                ALog.Trace($"[AsakiFrameworkSetting] File Changed: {fileName}, Reloading...");
                    ...
                    if (method != null)
                    {
                        ...
                        if (taskObj is Task task)
                        {
                            ...
                            Task.Run(async () =>
                            {
                                ...
                                {
                                    ALog.Error(
                                        $"[AsakiFrameworkSetting] Failed to reload '{fileName}'. Error: {ex.Message}",
                                        ex
                                    );
                                }
                            });
        ...
        ///     <item>回退到遍历所有类型，检查名称和<see cref="IAsakiDataTable"/>接口实现</item>
        /// </list>
        ...
        private Type FindConfigType(string typeName)
        {
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                {
                    if (t.Name == typeName && typeof(IAsakiDataTable).IsAssignableFrom(t))
                        return t;
                }
            }
            return null;
