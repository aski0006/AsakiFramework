...
namespace Asaki.Unity.Services.Audio
{
    ...
    public class AsakiAudioService : IAsakiAudioService
    {
        ...
        {
            // 1. 状态检查
            if (_resourceService == null || _poolService == null || _rootTransform == null)
            ...
            AsakiAudioParams finalParams = p.SetVolume(finalVolume)
                .SetPitch(finalPitch)
                .SetLoop(finalLoop);
            // 3. 从池中生成 Agent
            // 使用 _agentAssetKey 作为 Pool Key
            GameObject go = _poolService.Spawn(
                _agentAssetKey,
                Vector3.zero,
                Quaternion.identity,
                _rootTransform
            ...
            CancellationToken linkedToken = CancellationTokenSource
                .CreateLinkedTokenSource(_serviceCts.Token, token)
                .Token;

            // 这里传入 _poolService 和 _agentAssetKey，以便 Agent 播放完后能归还自己
            PlayInternal(agent, handle, path, finalParams, linkedToken, _agentAssetKey)
                .FireAndForget(ex =>
                {
