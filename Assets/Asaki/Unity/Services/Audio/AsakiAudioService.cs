...
namespace Asaki.Unity.Services.Audio
{
	...
	public class AsakiAudioService : IAsakiAudioService
	{
		...
		public async UniTask OnInitAsync()
		{
			...
			ALog.Info($"[AsakiAudioService] Agent pool initialized, stats: {_agentPool.Statistics}");
		}
		...
		)
		{
			...
			try
			{
				...
				ALog.Info($"[AsakiAudioService] Agent registered, active count: {_activeAgents.Count}");
			...
			finally
			{
				...
				ALog.Info($"[AsakiAudioService] Agent removed from active list, count: {_activeAgents.Count}");

				// ✅ Return to pool
				if (agent != null)
				{
					_agentPool.Return(agent);
					ALog.Info($"[AsakiAudioService] Agent returned to pool: {agent.GetInstanceID()}");
				}
			}
		}
	...
	internal class ComponentFactoryWrapper : IAsakiPoolObjectFactory<AsakiSoundAgent>
	{
		...
		public async UniTask<AsakiSoundAgent> CreateAsync(CancellationToken token = default)
		{
			...
			if (agent == null)
			{
				ALog.Error("[ComponentFactoryWrapper] AsakiSoundAgent component missing on prefab");
				Object.Destroy(go);
				return null;
			}
