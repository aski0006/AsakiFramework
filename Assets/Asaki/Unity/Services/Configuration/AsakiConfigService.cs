...
namespace Asaki.Unity.Services.Configuration
{
    public class AsakiConfigService : IAsakiConfigService
    {
        ...
        private readonly Dictionary<Type, Dictionary<int, IAsakiConfig>> _configStore =
            new Dictionary<Type, Dictionary<int, IAsakiConfig>>();
            ...
            where T : class, IAsakiConfig, new()
        {
            if (!IsLoaded<T>())
            {
                ...
                {
                    ALog.Error(
                        $"[AsakiConfig] {typeof(T).Name} requires manual loading.  Call LoadAsync<{typeof(T).Name}>() first."
                    );
                    return null;
                }

                ALog.Warn(
                    $"[AsakiConfig] {typeof(T).Name} not loaded, blocking load on main thread.  Consider using GetAsync or Preload."
                );
            ...
            where T : class, IAsakiConfig, new()
        {
            ...
            {
                ALog.Error(
                    $"[AsakiConfig] Failed to get modified time for {typeof(T).Name}: {ex.Message}",
                    ex
                );
        ...
        {
            if (!Directory.Exists(_csvRootPath))
                return;
            ...
            where T : class, IAsakiConfig, new()
        {
            ...
            if (File.Exists(binaryPath))
            {
                if (_isEditor)
                {
                    ...
                    {
                        ALog.Warn(
                            $"[AsakiConfig] Detected stale binary for '{fileName}'. Re-baking from CSV..."
                        );
                    }
                }
                else
            ...
            if (shouldLoadBinary)
            {
                ...
                {
                    ALog.Error(
                        $"[AsakiConfig] Failed to load binary '{fileName}', falling back to CSV. Error : {ex.Message}",
                        ex
                    );
                    results = null;
                }
        ...
        private async UniTask ValidateAllConfigsAsync()
        {
            ...
            {
                ALog.Error(
                    $"[AsakiConfig] ❌ Validation completed with {errors.Count} errors and {warnings.Count} warnings in {sw.ElapsedMilliseconds}ms."
                );
            }
            else if (warnings.Count > 0)
            {
                ALog.Warn(
                    $"[AsakiConfig] ⚠️ Validation completed with {warnings.Count} warnings in {sw.ElapsedMilliseconds}ms."
                );
            }
            else
            {
                ALog.Info(
                    $"[AsakiConfig] ✅ All {allTypes.Count} configs validated successfully in {sw.ElapsedMilliseconds}ms."
                );
            }
        }
