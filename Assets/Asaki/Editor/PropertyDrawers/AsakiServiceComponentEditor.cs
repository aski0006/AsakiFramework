...
namespace Asaki.Editor.PropertyDrawers
{
	...
	public class AsakiServiceComponentEditor : UnityEditor.Editor
	{
		...
		private void DrawDependencyFoldout()
		{
			...
			// 显示依赖来源信息
			Type targetType = target.GetType();
			MethodInfo injectMethod = targetType.GetMethods(
				                                    BindingFlags.Instance |
				                                    BindingFlags.Public |
				                                    BindingFlags.NonPublic)
		...
		{
			MethodInfo tryGetMethod = typeof(AsakiContext).GetMethod("TryGet", BindingFlags.Public | BindingFlags.Static);
		...
		{
			MethodInfo tryGetMethod = typeof(AsakiSceneContext).GetMethod("TryGet", BindingFlags.Public | BindingFlags.Instance);
			if (tryGetMethod == null) return false;
