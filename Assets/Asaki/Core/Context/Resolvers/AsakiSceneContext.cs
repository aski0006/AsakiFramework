...
namespace Asaki.Core.Context.Resolvers
{
	...
	public class AsakiSceneContext : MonoBehaviour, IAsakiResolver
	{
		...
		private void Awake()
		{
			ALog.Info($"[AsakiSceneContext] Registering services in scene: {gameObject.scene.name}");
			...
			ALog.Info($"[AsakiSceneContext] Registered {_localServices.Count} services. Waiting for Build()...");
		}
		...
		public void Build()
		{
			...
			ALog.Info($"[AsakiSceneContext] Building {_pendingInitServices.Count} pending services...");
			...
			foreach (var service in _pendingInitServices)
			{
				...
				{
					ALog.Error($"[AsakiSceneContext] Failed to init service {service.GetType().Name}: {ex}");
				}
			}
		...
		{
			if (_pureCSharpServices == null || _pureCSharpServices.Count == 0) return;
		...
		{
			if (_behaviourServices == null || _behaviourServices.Count == 0) return;
