...
namespace Asaki.Core.Architecture
{
	public abstract class AsakiArchitecture : IAsakiArchitecture, IAsakiInit
	{
		...
		public void Init(IAsakiResolver resolver)
		{
			...
			_isInited = true;
			ALog.Info($"[AsakiArchitecture] {GetType().Name} initialized. (M:{_models.Count}, S:{_systems.Count})");
		}
		protected abstract void OnSetup();
		protected void RegisterModel<T>(T model) where T : class, IAsakiModel
		{
			if (model == null) return;
			Type type = typeof(T);
			if (!_models.TryAdd(type, model))
			{
				ALog.Warn($"[AsakiArchitecture] Model {type.Name} is already registered in {GetType().Name}.");
			}

		}
		protected void RegisterSystem<T>(T system) where T : class, IAsakiSystem
		{
			if (system == null) return;
			Type type = typeof(T);
			if (!_systems.TryAdd(type, system))
			{
				ALog.Warn($"[AsakiArchitecture] System {type.Name} is already registered in {this.GetType().Name}.");
			}

		}
		public T GetSystem<T>() where T : class, IAsakiSystem
		{
			if (_systems.TryGetValue(typeof(T), out IAsakiSystem system))
			{
				return (T)system;
			}
			ALog.Error($"[AsakiArchitecture] Failed to get System: {typeof(T).Name}. Make sure it is registered in OnSetup().");
			return null;
		}
		public T GetModel<T>() where T : class, IAsakiModel
		{
			...
			ALog.Error($"[AsakiArchitecture] Failed to get Model: {typeof(T).Name}. Make sure it is registered in OnSetup().");
			return null;
		}
