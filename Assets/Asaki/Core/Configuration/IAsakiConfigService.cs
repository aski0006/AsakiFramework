...
namespace Asaki.Core.Configuration
{
	public interface IAsakiConfigService : IAsakiModule
	{
		...
		UniTask ReloadAsync<T>() where T : class, IAsakiConfig, new();
		...
		UniTask<T> GetAsync<T>(int id) where T : class, IAsakiConfig, new();
		UniTask PreloadAsync<T>() where T : class, IAsakiConfig, new();
