...
namespace Asaki.Core.Graphs
{
    ...
    public abstract class AsakiGraphAsset : ScriptableObject
    {
        ...
        public void InitializeRuntime()
        {
            ...
            {
                if (!_nodeLookup.TryGetValue(edge.BaseNodeGUID, out AsakiNodeBase source))
                    continue;
                if (!_nodeLookup.TryGetValue(edge.TargetNodeGUID, out AsakiNodeBase target))
                    continue;
        ...
        /// </remarks>
        public T GetEntryNode<T>()
            where T : AsakiNodeBase
        {
            if (Nodes.Count == 0)
                return null;
        ...
        public T GetNextNode<T>(AsakiNodeBase current, string portName = "Out")
            where T : AsakiNodeBase
        {
        ...
        public virtual TGraph Clone<TGraph>()
