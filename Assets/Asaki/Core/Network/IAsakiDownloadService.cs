...
namespace Asaki.Core.Network
{
	...
	public interface IAsakiDownloadService : IAsakiService
	{
		...
		UniTask DownloadAsync(string url, string localPath, IProgress<AsakiDownloadProgress> progress = null, CancellationToken token = default(CancellationToken));
		...
		/// </remarks>
		UniTask<long> GetFileSizeAsync(string url, CancellationToken token = default(CancellationToken));
	}
}
