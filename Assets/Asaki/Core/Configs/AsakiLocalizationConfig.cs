...
namespace Asaki.Core.Configs
{
    ...
    public class AsakiLocalizationConfig
    {
        ...
        [field: SerializeField]
        public string RawCsvPath { get; private set; } =
            "Assets/Editor/RawData/RawLocalization.csv";

        [Tooltip("生成的配置表类名/文件名 (不带.csv后缀)")]
        [field: SerializeField]
        public string OutputTableName { get; private set; } = "LocalizationTable";

        [Tooltip("生成文件存放的相对路径 (通常在StreamingAssets下)")]
        [field: SerializeField]
        public string OutputRelativePath { get; private set; } = "Configs";
    }
}
