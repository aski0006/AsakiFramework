...
namespace Asaki.CodeGen.Generators
{
	public static class AsakiSaveGenerator
	{
		...
		private static bool IsConfigType(ITypeSymbol typeSymbol, Compilation compilation)
		{
			...
			// 修正：应为 Asaki.Core.DataTable.IAsakiDataTable
			var configInterface = compilation.GetTypeByMetadataName("Asaki.Core.DataTable.IAsakiDataTable");
		...
		private static string GenerateClassCode(INamedTypeSymbol symbol, bool isConfigType, Compilation compilation)
		{
			...
			// 获取版本号
			int version = GetVersion(symbol);
			
			// 获取所有标记了 [AsakiSaveMember] 的成员
			var members = GetSaveMembers(symbol);

			// 排序逻辑：优先按 Order 排序，Order 相同按名称排序
			members.Sort((a, b) =>
			{
			...
			// ========== 增强：为 Config 类型添加必要的命名空间 ==========
			if (isConfigType)
			{
				sb.AppendLine("using Asaki.Core.DataTable;"); // 用于 IAsakiDataTable 接口
			}
			sb.AppendLine();
			...
			// ========== 接口声明优化：Config 类型明确实现 IAsakiDataTable ==========
			string interfaceList = isConfigType 
				? "Asaki.Core.DataTable.IAsakiDataTable"   // IAsakiDataTable 已继承 IAsakiSavable
				: "IAsakiSavable";
			sb.AppendLine($"    partial {kind} {typeName} : {interfaceList}");
			sb.AppendLine("    {");
			...
			// 2. 生成版本号属性 (如果实现了IAsakiVersionedSavable)
			bool implementsVersioned = ImplementsIAsakiVersionedSavable(symbol, compilation);
			if (implementsVersioned)
			{
				GenerateVersionMethod(sb, version);
			}
		...
		private static void GenerateVersionMethod(StringBuilder sb, int version)
		{
			sb.AppendLine("        // [AsakiVersioned]");
			sb.AppendLine($"        public int GetDataVersion() => {version};");
			sb.AppendLine();
		}
		...
		private static void GenerateCloneMethod(StringBuilder sb, string typeName, List<ISymbol> members, Compilation compilation, bool isConfigType)
		{
			...
			if (isConfigType)
			{
				sb.AppendLine($"        Asaki.Core.DataTable.IAsakiDataTable Asaki.Core.DataTable.IAsakiDataTable.CloneConfig()");
				sb.AppendLine("        {");
		...
		private static bool IsAsakiProperty(ITypeSymbol type, out ITypeSymbol innerType)
        {
            ...
            bool implementsBase = type.AllInterfaces.Any(i => 
                i.Name == "IAsakiPropertyBase" || 
                i.ToDisplayString() == "Asaki.Core.Reactive.IAsakiPropertyBase");
		...
		private static int GetVersion(INamedTypeSymbol symbol)
		{
			...
			{
				if (attr.ConstructorArguments[0].Value is int version)
					return version;
			}
			
			foreach (var arg in attr.NamedArguments)
			{
				if (string.Equals(arg.Key, "Version", StringComparison.OrdinalIgnoreCase))
				{
					if (arg.Value.Value is int v) return v;
				}
			}
		...
		private static bool ImplementsIAsakiVersionedSavable(ITypeSymbol typeSymbol, Compilation compilation)
		{
			if (typeSymbol == null) return false;
			
			var versionedInterface = compilation.GetTypeByMetadataName("Asaki.Core.Serialization.IAsakiVersionedSavable");
			if (versionedInterface == null) return false;

			return typeSymbol.AllInterfaces.Any(i => SymbolEqualityComparer.Default.Equals(i, versionedInterface));
		}
